...
It adds an AnkiPenDown menu entity with options like:
    switching AnkiPenDown
...
__version__ = "1.4.6" # Fixed Eraser Undo/Redo
from aqt import mw
...
def ts_change_opacity():
    # This is now a legacy setting as opacity is per-stroke, but we keep the menu option for now.
    showWarning("Opacity is now set per-tool (Pen vs. Highlighter). This setting is deprecated.")
...
def ts_load():
    ...
    """
    global ts_state_on, ts_pen1_color, ts_pen2_color, ts_profile_loaded, ts_line_width, ts_opacity, ts_auto_hide, ts_auto_hide_pointer, ts_default_small_canvas, ts_zen_mode, ts_follow, ts_orient_vertical, ts_y_offset, ts_x_offset, ts_location, ts_small_width, ts_small_height, ts_background_color
    try:
...
def ts_onload():
    ...
    ts_setup_menu()
def blackboard():
    # This function is rebuilt to avoid the f-string SyntaxError.
    # It constructs the HTML string in parts.
    
    # Part 1: Static HTML up to the first customizable button
    part1 = u"""
<div id="canvas_wrapper">
    ...
    <div id="pencil_button_bar">
        <!-- SVG icons from https://github.com/Rytisgit/Anki-StylusDraw -->
...
"""
    # Part 2: Pen 1 button, using an f-string for the color
    pen1_button = f"""
        <button id="ts_black_pen_button" class="color-button active" title="Pen 1"
...
"""
    # Part 3: Pen 2 button, using an f-string for the color
    pen2_button = f"""
        <button id="ts_red_pen_button" class="color-button" title="Pen 2"
...
"""
    # Part 4: The rest of the HTML, CSS, and JS, using standard strings and concatenation
    # This is the same method used in the original add-on, which is safe.
...
#pen_canvas, #highlighter_canvas {
  opacity: 1.0; /* Opacity is now controlled per-stroke via globalAlpha */
  border-style: none;
  border-width: 1px;
}
#pencil_button_bar {
...
/* For non-color buttons, make the icon path black/white when active */
...
/* For active color buttons, add a drop shadow to indicate it's active. */
...
//Initialize event listeners on the top canvas
...
function resize() {
    ...
    [pen_ctx, highlighter_ctx].forEach(function(ctx) {
        ...
        ctx.scale(dpr, dpr);
        ctx.lineJoin = 'round'; // Use round joins for smooth corners on both tools
    });
    ts_redraw();
}
...
function update_pen_settings(){
    // Legacy function, properties are now set per-stroke.
    // Kept to avoid breaking calls from python side.
}
function ts_undo(){
    stop_drawing();
    ...
    // If the undone stroke was an eraser, restore visibility of the strokes it erased
...
function ts_redo() {
    ...
    // If the redone stroke was an eraser, hide the strokes it originally erased
...
async function draw_path_at_some_point_async(active_ctx, startX, startY, midX, midY, endX, endY, lineWidth) {
		...
		active_ctx.moveTo((startX + (midX - startX) / 2), (startY + (midY - startY)/ 2));//midpoint calculation for x and y
...
async function draw_upto_latest_point_async(startLine, startPoint){
	...
	for(var i = startLine; i < strokes_data.length; i++){
        ...
        if (stroke.tool === 'eraser') {
            // Do not draw the eraser stroke on the canvas.
            continue;
        } else {
...
// Function to check if an eraser stroke intersects with a target stroke
function doesStrokeIntersectEraser(targetStroke, eraserStroke) {
    // Check each line segment in the target stroke against each line segment in the eraser stroke
...
// NEW: Function to hide strokes that intersect with the eraser stroke
...
def ts_setup_menu():
    ...
    # Create the new sub-menu for pen colors
    ...
    # Add the new sub-menu to the main menu
